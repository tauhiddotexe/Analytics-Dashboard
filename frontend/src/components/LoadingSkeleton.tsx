import { IconChart, IconBolt } from '../lib/icons';
    ...
    <div className="space-y-6" role="status" aria-label="Loading">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="card-apple px-4 py-4 sm:px-5 sm:py-5 relative overflow-hidden">
            <IconBolt size={60} className="absolute -bottom-3 -right-3 text-[#e5e5ea] opacity-50" strokeWidth={1} />
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="card-apple p-4 sm:p-5 relative overflow-hidden">
            <IconChart size={50} className="absolute -bottom-2 -right-2 text-[#e5e5ea] opacity-50" strokeWidth={1} />
